...
bool FrameEncoder::init(Encoder *top, int numRows, int numCols)
{
    ...
    ok &= !!m_sliceBaseRow;
    m_sliceGroupSize = (uint16_t)(m_numRows + m_param->maxSlices - 1) / m_param->maxSlices; //maxSlices=1，m_sliceGroupSize=m_numRows
    ...
    if (m_param->bEnableWavefront)
    {
        ...
        for (uint32_t rowInSlice = 0; rowInSlice < m_sliceGroupSize; rowInSlice++)
        {
            for (uint32_t sliceId = 0; sliceId < m_param->maxSlices; sliceId++)
            {
                ...
                if (row > sliceEndRow)
                    continue;
                m_row_to_idx[row] = i;  //CTU行索引
                m_idx_to_row[i] = row;  //CTU索引到行
                i += 1;
            }
        }
    }
...
void FrameEncoder::processRow(int row, int threadId)
{
    ...
    const uint32_t realRow = m_idx_to_row[row >> 1];  //获取当前真正的CTU行号
    const uint32_t typeNum = m_idx_to_row[row & 1]; //0 表示需要编码  1表示需要滤波

    if (!typeNum)
        processRowEncoder(realRow, m_tld[threadId]);  //波前并行方式编码
    else
    {
...
{
    const uint32_t row = (uint32_t)intRow; //获取当前CTU行号
